...
for a skill generated by `/pstack:create-verification-skill` (or any project-local
...
0. **Locate the target.** Find the verification skill to maintain: the
   ...
   (usually `.claude/skills/verify-*/`). Several candidates -> ask which one.
   None -> stop and point at `/pstack:create-verification-skill` instead of inventing
   a target.
