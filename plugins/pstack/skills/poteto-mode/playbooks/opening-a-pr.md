...
**Worktree.** Work from a Git worktree off main. Give every concurrent writing subagent `isolation: "worktree"` so no two share a checkout. Preserve dirty or unrelated work. Move the scoped patch to a fresh worktree when needed; never reset user state to clean up a lane.
