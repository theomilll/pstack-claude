...
2. Pick the wake mechanism using Claude Code's `/loop` command (a built-in, not a pstack skill). An event to watch (CI, a merge, a ref advancing) gets a `Monitor` on that event or a watcher subagent that wakes you, with a long time-based `/loop` heartbeat as fallback. No event gets a fixed-interval `/loop` sized to when the result is worth re-checking.
