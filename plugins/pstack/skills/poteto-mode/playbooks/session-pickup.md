...
1. Locate the prior trail. A session id resolves to `~/.claude/projects/<project-slug>/<session-id>.jsonl`, with its subagents under `<session-id>/subagents/`; `claude --resume <session-id>` reopens it. Verify its opening message before trusting it. A pushed branch is also a valid trail. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
