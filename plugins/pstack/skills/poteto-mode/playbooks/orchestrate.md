...
- **Coordinator (this task).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Subagents are spawned, steered, inspected, and stopped only through `Agent`, `SendMessage`, `ListAgents`, and `TaskStop`; completions arrive as task notifications. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, and spawns its own workers and verifiers when the subagent depth limit allows nested delegation. Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, as a rolling window within the concurrent-subagent limit. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Every writer spawns with `isolation: "worktree"` and reports its worktree path and branch. Read-only verifiers may share a checkout. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier as a fresh `pstack:read-only` subagent with a self-contained brief.
...
Create the store at `~/.claude/pstack/<project-slug>/orchestrate/`. Every file has exactly one writer. Owners publish facts, readers aggregate at read time. Use `bun scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it through `/loop` or a `Monitor` on the watcher's output), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Never start another subagent turn just to check liveness. Probe read-only through `ListAgents`, the ledger, `units.tsv`, `gh`, and pushed branches. Transcript mtime is not liveness.
...
- After a task restart, re-read the standing orders and `units.tsv`, inspect `ListAgents` and `/tasks`, recompute the frontier, and reattach durable work by PR and branch rather than assuming a child id still runs. Respawn one sub-coordinator per unfinished track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
