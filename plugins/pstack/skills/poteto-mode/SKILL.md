...
- Any prose surface -> the **unslop** rules in `../unslop/SKILL.md`. Read that
  file directly; the skill itself runs only when the user invokes
  `/pstack:unslop`. Your reply is a prose surface. Write it per **Writing the
  reply**. Subagent-facing prose also follows
  `playbooks/authoring-a-skill.md`.
- Docs, RFCs, readmes, PR descriptions, or commit messages -> the
  **technical-writing** skill (`/pstack:technical-writing`).
...
- writing lanes run as `general-purpose` and are told to invoke
  `/pstack:poteto-mode` or the relevant routed skill before any work
