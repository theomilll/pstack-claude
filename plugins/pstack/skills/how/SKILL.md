...
Decompose the question into 2 to 4 exploration angles, each a distinct slice of the subsystem. Spawn all explorers in a single message as `pstack:read-only` subagents. Follow `../poteto-mode/references/claude-delegation.md`.
...
Once all explorers have returned, spawn one fresh `pstack:read-only` subagent to synthesize their findings into one explanation.
