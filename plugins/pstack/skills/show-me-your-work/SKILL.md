...
At the end of the run, before handing back, check the log told the truth. Read this session's transcript, `~/.claude/projects/<project-slug>/${CLAUDE_SESSION_ID}.jsonl`, and verify its opening message. Never search another session. Walk the log against what actually happened:
...
Before handing back, spawn a fresh `pstack:read-only` subagent with a
self-contained brief. Self-review is not a substitute. Follow `../poteto-mode/references/claude-delegation.md`.
