...
named `mcp__<server>__<tool>`. Enumerate them with `ToolSearch` (query
`mcp__`) or `claude mcp list` in Bash.
...
Run each investigator as a `pstack:read-only` subagent so it cannot widen
scope. That agent keeps MCP access. Follow `../poteto-mode/references/claude-delegation.md`.
...
Spawn one fresh synthesizer as a `pstack:read-only` subagent.
