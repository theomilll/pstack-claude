...
Transcripts live at `~/.claude/projects/<slug>/<session-id>.jsonl`, with subagent transcripts under `~/.claude/projects/<slug>/<session-id>/subagents/`, where `<slug>` is the working directory with each "/" turned into "-" (so `/Users/you/proj` becomes `-Users-you-proj`). Every line is one JSON record; chat messages carry `"type":"user"` or `"type":"assistant"`.
