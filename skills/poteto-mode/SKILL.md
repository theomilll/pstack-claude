...
- Before commit → Claude Code's built-in `/simplify` over the diff. It strips reuse misses, dead weight, and padding; it does not hunt bugs.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the project's verification skill (`verify-<app>`, generated by `/create-verification-skill`). Without one, drive the surface yourself: the CLI in Bash, the browser through Claude in Chrome when it is connected. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
**Defaults for every `Agent` call.** Subagents run in the background and notify you when they finish; there is no flag to set. Read-only roles use `subagent_type: "pstack:read-only"` (it keeps MCP access; it drops Edit and Write). Roles that write use `pstack:poteto-agent` or `general-purpose`, with `isolation: "worktree"` whenever two writers could touch one tree. File pointers not inlined context. Explicit model per role (configurable via `/setup-pstack`; defaults `gpt-5.6-sol` for code, `fable` for prose and judgment). A Claude alias (`fable`, `opus`, `sonnet`, `haiku`) goes in the Agent `model` field. `gpt-5.6-sol` is not an Agent model: run that role as `subagent_type: "codex:codex-rescue"` with no `model` field, and say "read-only" in the prompt when the role must not write. Without the `codex` plugin, run code roles on `opus` and drop `gpt-5.6-sol` from panels. The full Cursor-to-Claude Code mapping is `HARNESS.md` at the plugin root. Playbooks write `<pstack root>` for that installed plugin directory (`~/.claude/plugins/cache/pstack-claude/pstack/<version>/`; resolve it once with `ls -d ~/.claude/plugins/cache/pstack-claude/pstack/*/`) and "the store" for `~/.claude/pstack/<project-slug>/`, durable state outside the repo. Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model (`fable`) when the task needs judgment or the intent is vague, and to your strongest instruction-following model (`gpt-5.6-sol`) when the work is a precisely specified sequence of steps to execute to the letter; trivial mechanical edits go to your fast code model. Per-role lines in the `/setup-pstack` rule (`~/.claude/rules/pstack-models.md`) override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`); a role with no line keeps its default, and a role line of `inherit-parent` or `auto` runs that role on the parent chat model (omit Agent `model`).

You own every subagent's work. Review the diff and write your own summary, don't pass through what it said. Resume a finished subagent with `SendMessage` to its id. Interrupt-chained resumes silently drop directives, so fire a fresh subagent with consolidated scope rather than trusting a "done" summary. A second opinion is the same prompt against a different model. Agreement is high-signal.
...
- **Session pickup.** Resuming or taking over a prior agent's in-flight work from a transcript, a session id, or a pushed branch. `playbooks/session-pickup.md`.
