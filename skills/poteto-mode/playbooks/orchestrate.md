...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder in the standing orders. Restacks run one at a time in the stacker's own worktree. A local restack at this scale takes the laptop down.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, background agent status in `/tasks`. Transcript mtime is not liveness.
