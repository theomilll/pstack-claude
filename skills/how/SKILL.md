...
- `subagent_type`: `pstack:read-only` (`codex:codex-rescue` with "read-only" in the prompt when the model is `gpt-5.6-sol`)
- `model`: your configured how-explorer model (default `gpt-5.6-sol`; omit for `codex:codex-rescue`)
...
After the explanation is complete, spawn one architectural critic per model in your configured how-critics list (defaults `fable`, `gpt-5.6-sol`, `opus`), all in a single message.

For each critic:
- `subagent_type`: `pstack:read-only` (`codex:codex-rescue` with "read-only" in the prompt for the `gpt-5.6-sol` entry)
- `model`: one model from the configured how-critics list (omit for `codex:codex-rescue`). The lead should escalate any model when the architecture warrants deeper analysis.
