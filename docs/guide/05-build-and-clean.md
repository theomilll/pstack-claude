...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs Claude Code's built-in `/simplify` on the diff before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. `/simplify` is a Claude Code built-in, not a pstack skill. It hunts reuse misses, dead weight, and padding, not bugs.
